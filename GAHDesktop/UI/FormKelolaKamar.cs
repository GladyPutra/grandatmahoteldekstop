...
namespace GAHDesktop.UI
{
    public partial class FormKelolaKamar : Form
    {
        ...
        public void setDatagridview(DataGridView DG)
        {
            ...
            DG.Columns[1].HeaderText = "Nama_Kamar";
            DG.Columns[2].HeaderText = "Harga_Kamar";
        ...
        {
            MySqlConnection cn = new MySqlConnection("server=localhost;uid=root;pwd=;database=gahdb");
                cn.Open();
        ...
        private void btnSimpan_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtHargaKamar.Text.Length > 0 && cmbNamaKamar.Text.Length > 0 && cmbSeason.Text.Length > 0 && dateTimePicker1.Text.Length > 0 && dateTimePicker2.Text.Length > 0)
                {
                    ...
                    Tarif_Kamar T = new Tarif_Kamar(tgl_mulai, tgl_selesai, int.Parse(id_season), int.Parse(id_detilKamar), double.Parse(harga_kamar));
        ...
        public void setDatagridview2(DataGridView DG, string temp)
        {
            ...
            DG.Columns[0].HeaderText = "ID_TARIF";
            DG.Columns[1].HeaderText = "NAMA_KAMAR";
            DG.Columns[2].HeaderText = "TGL_MULAI";
            DG.Columns[3].HeaderText = "TGL_SELESAI";
            DG.Columns[4].HeaderText = "HARGA_KAMAR";
            DG.Columns[5].HeaderText = "NAMA_SEASON";
        ...
        {
            DialogResult dr = MessageBox.Show("Apakah anda yakin akan logout", "Pertanyaan", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void txtCari_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    temp = txtCari.Text;
                    if (temp.ToLower() == "junior suite" || temp.ToLower() == "superior" || temp.ToLower() == "double deluxe" || temp.ToLower() == "executive deluxe")
                    {
                        RefreshGridView2(temp);
                    }
                    else
                    {
